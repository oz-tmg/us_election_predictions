...
| P0-002 | Create canonical geography table | Prevents FIPS/GEOID/district mismatch. | State, county, district, precinct, media market keys documented. | todo |
...
| P0-004 | Build source manifest schema | Enables reproducibility and legal review. | Every raw source snapshot has source, date, checksum, license, privacy tier. | todo |
| P0-005 | Ingest MIT/MEDSL federal returns | Core historical baseline. | President, House, Senate available in standardized silver tables. | todo |
| P0-006 | Ingest Census ACS features | Core demographics. | ACS variables selected, transformed, and joined to geography table. | todo |
| P0-007 | Ingest TIGER/Line boundaries | Core geospatial layer. | Current state/county/CD boundaries stored in PostGIS/GeoParquet. | todo |
| P0-008 | Build model-ready race table | Central model grain. | One row per race/candidate or race/party with results and metadata. | todo |
| P0-009 | Build data-quality report | Makes gaps visible. | Missingness, duplicate keys, vote-total reconciliation, stale sources. | todo |

## P1 — Baseline Forecasting

| ID | Task | Analytical Question | Acceptance Criteria | Status |
|---|---|---|---|---|
| P1-001 | Presidential fundamentals model | How much can we predict without polls? | Backtest by state for 2008–2024 with MAE and calibration. | todo |
| P1-002 | House district baseline | What is each district's normal partisan lean? | District partisanship score using presidential and House history. | todo |
...
| P1-005 | Correlated simulation layer | How does race-level uncertainty translate to seat control? | Simulation returns win probability, seat distribution, chamber probability. | todo |
| P1-006 | Forecast evaluation notebook | Are probabilities calibrated? | Brier score, log score, calibration curve, interval coverage. | todo |
...
| F-002 | Past presidential vote | All geographic races | Strong baseline for partisanship. | Latest and previous presidential two-party vote by geography. | todo |
| F-003 | District partisanship score | House, State Leg | Core prior. | Standardized score with cycle and plan version. | todo |
...
| F-006 | Demographics | All | Explains geography and MRP poststrata. | Age, race/ethnicity, education, income, urbanicity. | todo |
