...
| `special_elections_compiled` | Hand-compiled special-election results 2025-2026 | Savepoint Analytics (compiled from state election boards + published trackers) | Derived / manual compilation | schema + validation built 2026-09-01; **no rows entered yet** | public_aggregate | State-legislative or congressional district | One special election | 2025-2026 | High | Compile per `docs/special-elections-sourcing.md`. Every row requires `source_url`, `retrieved_on`, and `baseline_source` or validation rejects it — there is no upstream checksum for hand-entered data. Feeds the national-environment estimate that P1-004 cannot supply for the 2022 plan era. |
| `ballotpedia_api` | Ballotpedia Data Client API | Ballotpedia | **Licensed, quote-on-request** | identified 2026-09-01 — **pricing not published**; terms not yet read | public_aggregate | District / candidate / officeholder | Varies by endpoint | Current + historical | Medium-High | Only complete source for specials *and* primary nominees. `developer.ballotpedia.org` documents geographic, ballot-measure, and bulk-data endpoints but publishes no rate card. `ballotpedia.org` returns HTTP 202 with an empty body to automated requests — **scraping is blocked and out of scope**; use the paid API under agreement or not at all. Human must read the Terms of Use (GitBook SPA, not machine-readable) before any commitment. |
| `downballot_split_ticket` | The Downballot / Split Ticket special-election trackers | The Downballot; Split Ticket | Open web (articles + spreadsheets) | identified 2026-09-01 (reachability confirmed; formats not inspected) | public_aggregate | District | One special election, with presidential baseline | 2025-2026 | High | Source for `baseline_dem_share` — they publish district presidential baselines this repo cannot compute below the congressional district. Cite per row. Free to read; The Downballot has a paid tier. |
| `mcdonald_pvld` | Presidential Vote within Legislative Districts | Michael P. McDonald (Harvard Dataverse doi:10.7910/DVN/24655) | Open **CC0** | identified 2026-09-01 — metadata read, files not downloaded | public_aggregate | State legislative district | District-cycle presidential vote | pre-2014 vintage | Low-Medium | Would close the district-baseline gap, but **released 2014-02-04**, so it predates both the 2012 and 2022 state-legislative redistricting rounds and cannot serve as a 2024 baseline. Useful for historical calibration of the overperformance metric only. |
...
| `fec_api` | FEC API Data | Federal Election Commission | Open API, **key required** (api.data.gov) | identified — **now on the critical path** | public_aggregate/public_pii_restricted | Candidate/committee/contributor geography | Contribution, disbursement, filing | Current + historical | High | Needed to fill `incumbent_status` and candidate rosters in `data/gold/race_universe_2026.parquet` (P0-001 ships the seat list without them) and for F-004/F-005. Obtain key, register permitted use, then build the filings join. Contributor fields are Tier 2: aggregate-only, never for solicitation. |
